...
/*
    for (int i_nuc=0;i_nuc<Movers[0]->nuclei_dist_step.size();++i_nuc){      
      std::ofstream myfile;
      char filename [50];
      sprintf(filename,"nuclei_distVMC.%i.txt",i_nuc+1);
...
void VMCSingleOMP::resetRun()
{
  ...
  {
    ParticleSet* ions=psiPool.getParticleSet("ion0");    
    std::vector<int> ionsToBeMoved;
    for (int i=0;i<ions->R.size();++i)
      {
	if (ions->Mass[i]>0.0)
	  ionsToBeMoved.push_back(i);

	IonStorage.push_back(ions->R[i]);
	//needed to give the original configuration details to DMC
    ...
    for(int ip=0; ip<NumThreads; ++ip)
    {
      ...
      else
      {
        ...
        {	  
          ...
          Movers[ip]=new VMCUpdateAllWithIons(*wClones[ip],*psiClones[ip],*hClones[ip],*Rng[ip],*ions,ionsToBeMoved);
