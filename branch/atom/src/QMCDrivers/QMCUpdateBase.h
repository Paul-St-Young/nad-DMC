...
//  //     assert (Tau==i);
  ...
  {    
    ...
    return -(GridLaplacianArray(wfs1,f1,h)/f1div)/2.0/ion_mass;    
  }


  inline RealType nuclei_wfs(ParticleSet::ParticlePos_t ionR)
  {
    // at T=0, atom wave function is a plane wave with k=0
    return 1.0;
  }

  inline PosType nuclei_wfs_gradient_div_nuclei_wfs(ParticleSet::ParticlePos_t ionR, int particle)
  {
    PosType dr(0.0);
    return dr;    
  }

  inline RealType nuclei_wfs_laplacian_div_nuclei_wfs(ParticleSet::ParticlePos_t ionR, int particle, PosType particle_grad)
  {
    return 0.0;    
  }
