  ...
  for(; wit != wit_end; ++wit)
    {
      ...
      for (int i=0;i<DMCIons.R.size();++i)
        {
          // Mass==1 is default for fixed nuclei (not used)                                     
    ...
    bool measure)
{
  for(; it != it_end; ++it)
  {
    ...
    PosType drift2[W.getNumIons()];
    RealType wfs[3][2];
    RealType wfs2[3][2];
    RealType h=0.000001;    
    RealType f=std::exp(logpsi1);
	...
	drift2[j]=ionDrift(wfs,h,f,Tau/DMCIons.Mass[j]/2.0);
      }
    ...
    for (int i=0;i<W.getNumIons();++i)
      {
	//PosType dr=(-std::sqrt(Tau/DMCIons.Mass[i])*deltaR[i]-2.0*drift2[i]);
	ilogGf += -dot(deltaR[i],deltaR[i])/2.0;
	//ilogGb += -dot(dr,dr)/2.0*DMCIons.Mass[i]/Tau;
	ionR[i]=ionR[i]+std::sqrt(Tau/DMCIons.Mass[i])*deltaR[i]+drift2[i];
	DMCIons.R[i] = ionR[i]; 
      }
