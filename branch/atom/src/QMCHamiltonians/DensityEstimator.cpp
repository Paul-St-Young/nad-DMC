...
DensityEstimator::DensityEstimator(ParticleSet& elns,ParticleSet& ionns) : rVs(0), myP(&ionns)
{
...
DensityEstimator::Return_t DensityEstimator::evaluate(ParticleSet& P)
{
  ...
  {
    for(int iat=0; iat<myP->getTotalNum(); ++iat)
    {
      PosType ru;
      for (int dim=0; dim<OHMMS_DIM; dim++)
      {
        ...
        ru[dim]=(myP->R[iat][dim]-density_min[dim])*ScaleFactor[dim];
      }
