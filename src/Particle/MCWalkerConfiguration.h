...
public:
  ...
  void createWalkers(int numWalkers, int nions);
  /** create walkers
  ...
  /*
  inline PosType getIonPos(int iat)
  {
    return ionStore[iat];
  }

  inline void setIonPos(PosType IonPosition, int iat)
  {
    ionStore[iat]=IonPosition;
  }

  inline void setIonPosAll(PosType IonPosition[])
  {
    for (int i=0;i<ionStore.size();++i)
      ionStore[i]=IonPosition[i];
  }

  inline int getNumIons() const
  {
    return ionStore.size();
  }

  inline void initializeIonPos(PosType IonPosition)
  {
    ionStore.push_back(IonPosition);
  }
  */
  ...
  void setNumSamples(int n, int nions);
  ...
  void loadEnsemble(vector<MCWalkerConfiguration*>& others, int nions, bool doclean=true);
 ...
 * $Id: MCWalkerConfiguration.h 5826 2013-05-07 01:02:09Z jnkim $
 ***************************************************************************/
