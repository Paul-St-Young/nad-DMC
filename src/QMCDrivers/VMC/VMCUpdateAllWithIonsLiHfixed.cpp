...
void VMCUpdateAllWithIons::advanceWalkers(WalkerIter_t it, WalkerIter_t it_end, bool measure)
{
  for (; it!= it_end; ++it)
  {
    ...
    RealType f2old=testGaussian(ionR[0],VMCIons.R[1],coeff,rc);
    for (int i=0;i<thisWalker.ionPos.size();++i)
      {
    ...
    RealType f2=testGaussian(ionR[0],VMCIons.R[1],coeff,rc);
	  ...
	  for (int i=0;i<3;++i)
	    {
	      ...
	      wfs2[i][0]=testGaussian(VMCIons.R[0],VMCIons.R[1],coeff,rc);
	      
	      VMCIons.R[j][i] = ionR[j][i]+h;
	      VMCIons.update();
	      W.update();	  
	      wfs[i][1]=std::exp(Psi.evaluateLog(W));
	      wfs2[i][1]=testGaussian(VMCIons.R[0],VMCIons.R[1],coeff,rc);
