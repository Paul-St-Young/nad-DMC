	...
	for (int i=0;i<3;++i)
	  {
	    ...
	    wfs2[i][0]=testGaussian(DMCIons.R[0],DMCIons.R[1],coeff,rc_oh)
	      +testGaussian(DMCIons.R[0],DMCIons.R[2],coeff,rc_hh)
	      +testGaussian(DMCIons.R[1],DMCIons.R[2],coeff,rc_oh);
	    
	    DMCIons.R[j][i] = ionR[j][i]+h;
	    DMCIons.update();
	    W.update();	  
	    wfs[i][1]=std::exp(Psi.evaluateLog(W));
	    wfs2[i][1]=testGaussian(DMCIons.R[0],DMCIons.R[1],coeff,rc_oh)
	      +testGaussian(DMCIons.R[0],DMCIons.R[2],coeff,rc_hh)
	      +testGaussian(DMCIons.R[1],DMCIons.R[2],coeff,rc_oh);
